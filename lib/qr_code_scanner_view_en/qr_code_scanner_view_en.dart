...
class QRCodeScannerView_en extends StatefulWidget {
  @override
  _QRCodeScannerViewState_en createState() => _QRCodeScannerViewState_en();
}

class _QRCodeScannerViewState_en extends State<QRCodeScannerView_en> {
  ...
  Future<void> _transitionToNextScreen(String data) async {
    ...
    if (!_isQRScanned) {
      ...
      await Navigator.push(
        context,
        MaterialPageRoute(
          builder: (context) => ScoreReadingEngWidget(ConfirmViewArguments(data: data))
        )
      ).then(
        // 遷移先画面から戻った場合カメラを再開
            (value) {
